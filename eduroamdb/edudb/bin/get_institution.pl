...
sub get_institution_xml {
  ...
  foreach my $realm (keys %$r_realms) {
    ...
    my $request = HTTP::Request->new( $method, $$r_arr{INST_XML} );

    if( $$r_lm{$realm} ) {
      # is filled only if cache file exist
      $request->header('If-Modified-Since', time2str($$r_lm{$realm}));
    }
    my $ua = LWP::UserAgent->new;
    my $response = $ua->request($request);

    # checks return code
    log_it(LOG_INFO, "get_institution_xml: Realm-id: $realm; http return code: ".$response->code);
    ...
    if ($response->is_success()) {
      ...
      my $temp_doc;
      eval {
	$temp_doc = $parser->parse_string( $response->content );
      };
	...
	unless ($err) {
	  ...
	  if (downloaded_diff_cached($filename, $temp_doc)) {
	    ...
	    my $last_modified = $response->last_modified || time;
	    if (open(F_LM, ">".$f_lm)) {
