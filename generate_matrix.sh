...
{
  echo "host_name,service_description,service_state,service_last_check" ;       # add csv header
  icingacli monitoring list services --service="*@*" --format='$host_name$,$service_description$,$service_state$,$service_last_check$' --columns=host_name,service_state,service_description,service_last_check |  # get data
